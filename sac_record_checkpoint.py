...
def main():
    ...
    # init wandb
    wandb.init(project="kerbal-rl")
    # log parameters
    wandb.config.lr_pi = lr_pi
    wandb.config.lr_q = lr_q
    wandb.config.init_alpha = init_alpha
    wandb.config.gamma = gamma
    wandb.config.batch_size = batch_size
    wandb.config.buffer_limit = buffer_limit
    wandb.config.tau = tau
    wandb.config.target_entropy = target_entropy
    wandb.config.lr_alpha = lr_alpha

    wandb.config.max_thrust_ratio = max_thrust_ratio
    wandb.config.max_altitude = max_altitude
    wandb.config.max_step = max_step
    wandb.config.max_speed = max_speed
    ...
    for n_epi in range(800):
        ...
        if n_epi%print_interval==0 and n_epi!=0:
            ...
            torch.save(pi.state_dict(), f"{wandb.run.dir}/pi_{n_epi}.pt")
            score = 0.0
